...
class ArizPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('ariz.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `ariz`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM ariz WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO ariz (date, title, link, content)"
